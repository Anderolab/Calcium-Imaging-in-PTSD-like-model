function [Output,SexPerNeuron,AnimalPerNeuron,ResponseType,TOI,TOR,STD_Distance, Responses] = F_GetTunningProps_Leire_Final(Experiment, TunningEpochs, ...
...
for an = Animals.'
    ...
    elseif startsWith(TunningEpochs, "ROI")
        ...
        for i = 1:length(Task.Titles)
            ...
            if startsWith(Task.Titles{i}, "NO_ROI") || startsWith(Task.Titles{i}, ReferenceEpochs)  
                TOR = [TOR, Task.Start(i):(Task.Start(i)+Task.Frames(i))];
