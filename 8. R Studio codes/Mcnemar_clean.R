...
# McNemar Test for paired data of CellReg, copy data from the excel results, be sure you write "active" from first test (OR)
#to "active/inactive/unresponsive" of the second (SI) test
...
# Composition OR vs SI by group (using row and column sums of each matrix)
compare_OR_SI <- function(M) {
  OR <- rowSums(M)              # active/inhibited/unresponsive in OR
  SI <- colSums(M)              # active/inhibited/unresponsive in SI
  rbind(OR = OR, SI = SI)
}

# ----------McNemar: binarizations ---------------------------------------------

#a) category vs. rest (generic)
...
run_all <- function(M_list, export_xlsx = TRUE, xlsx_path = "mcnemar_summary_OR_SI.xlsx") {
  # Composition OR vs SI by Chi-cuadrado/Fisher
  ...
  # McNemar tests in three binarizations
...
# ---------- Execute -----------------------------------------------------------
# Row-wise proportions (interpretable) for quick inspection
props <- lapply(M_list, function(M) round(prop_rows(M), 3))
# print(props$FV); print(props$FC); print(props$MV); print(props$MC)  # optional

out <- run_all(
  M_list,
  export_xlsx = TRUE,
  xlsx_path = "F:/CAIMAN/ENGLISH/7. R codes/mcnemar_summary_OR_SI_J_O.xlsx"
)


# Summary in the console, optional
