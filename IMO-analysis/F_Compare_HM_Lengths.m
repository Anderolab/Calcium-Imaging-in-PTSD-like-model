function [Out, Lengths, Events] = F_Compare_HM_Lengths(Data1, Data2, LowBound, HighBound, ...
...
for d_ix = 1:2
    ...
    for animal = Animals
        ...
        events = F_FindEvents(select_data, LowBound, ...
            HighBound, false);
        evs_{c} = events;
        Epochs(Animals == animal, d_ix) = mean(events(:, 2)-events(:, 1));
        len = [len; events(:, 2)-events(:, 1)];
        c = c+1;
    end
    Lengths{d_ix} = len;
    Events{d_ix} = evs_;
...
% And saving it
savename = strcat(save_path, "\Compare_HM_Lenghts.mat");
save(savename, "Out");
savename_chi = strcat(save_path, "\Compare_HM_Lenghts.csv");
...
NewFolderName = 'StatResults_HM_Lenghts'
