function [Out] = F_Compare_AUC(Data1, Data2, RF, GroupBy, AnimalReg, ...
    EpochNames, Visualise, Y_Label, ColorDict, save_path)
Groups = [];
Animals = [];
Epochs = [];
...
for d_ix = 1:2
    ...
    for animal = Animals
        ...
        % Computing the mean fluorescence for each animal at each toime
        % frame and then the AUC
        Epochs(Animals == animal, d_ix) = trapz(nanmean(select_data, 1) / ...
            (length(select_data)/RF));
...
% And saving it
savename = strcat(save_path, "\Compare_AUC_Table.mat");
save(savename, "Out");
savename_chi = fullfile(save_path, "Compare_AUC_Table.xlsx");
