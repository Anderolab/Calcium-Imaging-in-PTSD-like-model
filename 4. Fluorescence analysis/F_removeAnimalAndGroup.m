function Experiment = F_removeAnimalAndGroup(Experiment, animalsIndexToRemove)
    ...
    for i = 1:length(animalsIndexToRemove)
        ...
        % Eliminar la información del animal de la subestructura OF
        if isfield(Experiment.SPT, animalToRemove)
            Experiment.SPT = rmfield(Experiment.SPT, animalToRemove);
        end
    ...
    % Renombrar las claves de los animales restantes en SPT para reflejar los índices correctos
    animalFields = fieldnames(Experiment.SPT);  % Obtener los campos de animales actuales
    for i = 1:length(animalFields)
        correctIndex = sprintf('M%d', i);
        if ~strcmp(animalFields{i}, correctIndex)
            Experiment.SPT.(correctIndex) = Experiment.SPT.(animalFields{i});
            Experiment.SPT = rmfield(Experiment.SPT, animalFields{i});
        end
    end
end